...
### 3.2 객체 탐지 딥러닝 기반 기법
- 1단계 탐지 : 입력 이미지를 한번에 처리해 위치와 클래스를 동시에 예측합니다. 입력 이미지를 바로 격자나 앵커 박스 단위로 나누고, 각 영역에서 객체의 존재 여부와 클래스, 위치 좌표를 곧바로 예측합니다. 속도가 빠르고 실시간 처리에 유리하지만, 작은 객체나 복잡한 배경에서는 상대적으로 정확도가 떨어집니다.

<img width="940" height="238" alt="image" src="https://github.com/user-attachments/assets/e027db3d-25b2-412d-98ae-82d3599465c2" />

- 2단계 탐지 : 2단계 탐지 기법은 먼저 객체가 있을 법한 후보 영역(region proposal)을 찾고, 그 영역을 다시 분류, 정교화하는 방식입니다. 즉, 1단계에서 "여기 객체가 있을 수 있다"라는 후보 박스를 만들고, 2단계에서 그 후보를 세밀하게 분석해 최종 클래스와 위치를 결정합니다. 정확도가 높고 작은 객체 탐지에도 강하지만, 속도가 느려 실시간 처리에는 덜 적합합니다.

<img width="940" height="274" alt="image" src="https://github.com/user-attachments/assets/4bb3f097-aa92-4a95-a0a7-bfb72405900b" />

### 3.3 YOLO 모델

객체 탐지를 단일 회귀 문제로 재정의하여, 기존의 복잡한 파이프라인 대신 단일 신경망으로 이미지 픽셀에서 바운딩 박스와 클래스 확률을 실시간으로 예측합니다

1. 극도로 빠른 속도 : 복잡한 파이프라인 없이 단일 신경망 평가만으로 탐지가 가능하며 매우 빠릅니다. 기본 YOLO 모델은 Titan X GPU에서 실시간으로 초당 45프레임을 처리합니다
2. 전역적 추론 : 훈련 및 테스트 시 전체 이미지를 보기 때문에 클래스와 외형에 대한 맥락적 정보를 암묵적으로 인코딩합니다.
3. 일반화된 표현 학습 : 자연 이미지로 훈련하고 예술 작품(artwork)에서 테스트했을 때, DPM이나 R-CNN 같은 최고 수준의 탐지 방법보다 월등한 성능을 보이며 새로운 도메인에 대한 일반화 능력이 뛰어납니다.
...
YOLO에서는 어떤 방식으로 Bounding Box를 추정하고 물체를 분류하는지 예시 그림을 통해 단계별로 설명을 해보겠습니다.

1. 이미지를 n*n 형태로 나눕니다. 아래 그림에서는 7x7입니다.
2. 물체의 ground truth의 중심점이 포함된 이미지 조각은 물체가 있는 것으로 판단하고 만약 ground truth가 포함되지 않은 경우라면 바운딩 박스 예측을 하지 않습니다.
3. 하나의 cell에서 두 개의 bounding box를 랜덤하게 잡습니다.(보통 하나는 크고 하나는 작게), 7x7 에서는 49 x 2개 98개의 box가 나오게 됩니다.
4. 만약 물체가 있는 cell이라면 두개의 bounding box 중 실제 Bounding box에 더 가까운 box를 Ground truth와 최대한 겹치게 되도록 예측을 하고, 다른 하나는 예측하지 않습니다. 만약 물체가 없는 cell이라면 두 bounding box 모두 예측을 수행하지 않습니다.
5. 위 과정을 통해 총 98개의 박스를 Non-Max Suppression을 진행하여 ground truth에 유사한 박스만 남깁니다.
6. 최종적으로 물체 3개에 대한 박스만 남습니다.

<img width="596" height="600" alt="image" src="https://github.com/user-attachments/assets/7f495abe-3575-4295-b4b8-bc0b7570c6ba" />
